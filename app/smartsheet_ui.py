"""Reusable Streamlit components for manual Smartsheet form entry."""
...
<div id="ss-root" style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;color:#12233B;">
  <div style="display:flex;gap:8px;margin-bottom:10px;">
    ...
    <button id="ss-reset" style="border:1px solid #CBD5E1;background:#F8FAFC;border-radius:10px;padding:0 13px;font-weight:700;color:#475569;">Reset</button>
  </div>
  ...
  <div id="ss-list"></div>
  <button id="ss-copy-all" style="width:100%;margin-top:8px;border:1px solid #DDD6F3;background:#F5F3FF;color:#6D5AE6;border-radius:10px;padding:10px;font-weight:750;">
    Copy all fields as a list
  </button>
  <div id="ss-status" style="min-height:20px;margin-top:7px;font-size:12px;color:#64748B;"></div>
</div>
<script>
(() => {
  const D = __PAYLOAD__;
  ...
  function paint() {
    ...
    list.innerHTML = D.rows.map((row, index) => {
      ...
      return `<div style="display:flex;gap:9px;align-items:center;padding:9px 10px;margin-bottom:6px;border-radius:10px;border:${active ? '2px solid #6D5AE6' : '1px solid #E2E8F0'};background:${complete ? '#F0FDF4' : '#fff'};opacity:${complete ? '.75' : '1'};">
        ...
        <button data-index="${index}" style="border:1px solid ${active ? '#6D5AE6' : '#DDD6F3'};background:${active ? '#6D5AE6' : '#F5F3FF'};color:${active ? '#fff' : '#6D5AE6'};border-radius:8px;padding:7px 12px;font-weight:750;white-space:nowrap;">Copy</button>
      </div>`;
    }).join('');
